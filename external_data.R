...
#sec_2 <- read.dta("~/Documents/Modi Labs/Nigeria/Nigeria DATA/NIGERIA/Data/HNLSS/Raw_Data/section_2.dta")
...
literacy$literacy_rate <- 100*(literacy[,"count_liter"]/literacy[,"count_age"])
...
i_water$percentage_households_with_access_to_improved_water_sources <- 100*(i_water$count_improved/i_water$count_total) 
...
i_sanitation$percentage_households_with_access_to_improved_sanitation <- 
  100*(i_sanitation$count_improved/i_sanitation$count_total)
...
master <- read.xls("C:/Users/Zaiming/Dropbox/Nigeria 661 Baseline Data Cleaning (2)/external data/Nigeria Master Codes_SP.xlsx", colClasses="character")
...
write.csv(master,"C:/Users/Zaiming/Dropbox/Nigeria 661 Baseline Data Cleaning (2)/external data/hnlss_LGA_Reference_Sheet.csv", row.names=F)
...
lga_id <- read.csv("in_process_data/nmis/Education_661_ALL_FACILITY_INDICATORS.csv")
x_lga <- unique(lga_id$X_lga_id)
...
hiv_b <- merge(hiv, ref[,c("LGA_id", "lg_hnlss", "state_hnlss")], by.x=c('state', 'lg'), 
               by.y=c('state_hnlss', 'lg_hnlss'))
