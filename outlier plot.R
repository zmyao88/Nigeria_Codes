...
population <- function(df, lga_col, target_col, work_dir)
{
    ...
    df <- merge(df, pop, by.x=lga_col, by.y="X_lga_id")
...
#* Example: 
#* (L1): outdir="C:/Users/zmyao/Dropbox/Nigeria 661 Baseline Data Cleaning (2)/raw_data/"
...
#Plot function for printing only the first pareto chart
    ...
    {
        pdf(paste(output_dir, "plot.pdf", sep=""))
...
#    par(mar=c(5,4,4,5)+.1)
#    par(new=F)
#    plot(log(df[,"ratio"], 10), type="p", col="grey")
#    abline(h = quantile(log(df[,"ratio"],10), probs=perctl,na.rm=T), col="green" )
#   par(mar=c(0, 0, 0, 0))
#    legend("topleft",col=c("grey","green"),lty=1,lwd=3,cex=0.6,
#           legend=c(paste("Log 10 of ratio of", target_col, "vs population", sep=" "),
#                    paste(perctl*100, "% Percentile line of Log raito", sep="")))
...
#edu$num_students_total_gender.num_students_female
#pareto(edu,"num_students_total_gender.num_students_female",lga_col="X_lga_id")    
#pareto(health, "total", perctl=0.995)